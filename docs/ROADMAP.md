...
2. Build and exercise a reproducible installer and complete stock-recovery path.
   The desktop runtime, source-only hardware orchestration, and reproducibly
   built ARM64 compatibility packages are complete; compiled hardware daemons,
   locally generated stock assets, generic-root assembly, and the destructive
   rehearsal remain.
