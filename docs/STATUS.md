...
| S Pen pressure | Not yet accepted in native GNOME | Physical sensor pressure and varying GTK pressure before GNOME are confirmed. Inner native GTK receives finger events but no accepted pen samples. Manual proximity preloads were removed. The isolated Mutter trace build starts with verified password locking; a separately opt-in real-event/source-axis handoff trial compiled, but physical pressure and hover-out acceptance remain. See the [pressure-path report](reports/pen-pressure-path-20260915.md) |
