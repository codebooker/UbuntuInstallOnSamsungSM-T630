...
| Release packaging | In progress | The native source set is dependency-consistent under `t630-release-base` 0.1.19; desktop 0.1.10 keeps the guarded Ubuntu **Restart into Android** button and removes retired Waydroid hooks. The 3.5 GB identity-clean root passes the complete package, identity, application, linkage, Waydroid-retirement, and mount-leak gates. Its 1,230,453,239-byte private archive was sealed with the exact accepted dual-layout Ubuntu BOOT, copied into bounded recovery RAM, and reverified. Check mode refused mounted `linuxroot`; the new orderly preparation helper then preserved RAM while unmounting it, and the physical no-write check passed the exact split geometry plus BOOT/recovery/vendor_boot/DTBO/VBMETA guards. The exact DZE3/XAR factory ZIP also has a deep streaming verifier. Destructive clean-install/first-boot and return-to-stock acceptance remain behind their explicit gates. See the [dual-layout installer report](reports/dual-layout-installer-gate-20260917.md). |
