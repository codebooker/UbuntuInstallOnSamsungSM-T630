...
| Android applications | Native Android working; button switchers implemented, physical acceptance pending | Waydroid remains stopped because its llvmpipe UI is unusably slow. The physical split provides 64 GiB Ubuntu `linuxroot` plus 44.2 GiB Android `userdata`. Stock Android 15 uses encrypted `/data`, Adreno 642L acceleration, Play Store/services, and Samsung Notes S Pen drawing. The guarded Download-Mode BOOT-only return and a repeat Ubuntu-to-Android switch pass. Ubuntu now has **Restart into Android** in Quick Settings/Tablet Controls, and a no-network Android app provides **Restart into Ubuntu** through a fail-closed root helper. Installing the rooted service and repeating button-only cold/failure cycles remain. See [WAYDROID.md](WAYDROID.md), [DUALBOOT.md](DUALBOOT.md), and the [first-boot report](reports/native-android-firstboot-20260916.md). |
