...
| Android applications | Native Android working; button-only round trip accepted | Waydroid remains stopped because its llvmpipe UI is unusably slow. The physical split provides 64 GiB Ubuntu `linuxroot` plus 44.2 GiB Android `userdata`. Stock Android 15 uses encrypted `/data`, Adreno 642L acceleration, Play Store/services, and Samsung Notes S Pen drawing. Ubuntu's **Restart into Android** confirmation and the no-network Android **Restart into Ubuntu** app completed a physical touchscreen round trip. Android retained encrypted data and the rooted switch service; Ubuntu returned on its exact accepted BOOT with GNOME and the reverse helper ready. Longer cold-cycle and forced-failure endurance still remain. See [WAYDROID.md](WAYDROID.md), [DUALBOOT.md](DUALBOOT.md), the [first-boot report](reports/native-android-firstboot-20260916.md), and the [button-only round-trip report](reports/native-button-dualboot-20260917.md). |
