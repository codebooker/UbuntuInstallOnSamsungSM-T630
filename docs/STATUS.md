...
| Android applications | Native Android working; standalone switching remains | Waydroid remains stopped because its llvmpipe UI is unusably slow. The physical split provides 64 GiB Ubuntu `linuxroot` plus 44.2 GiB Android `userdata`. Stock recovery initialized Android F2FS/metadata, stock Android 15 completed setup with encrypted `/data`, Adreno 642L acceleration, Play Store/services, and Samsung Notes S Pen drawing, and the guarded Download-Mode BOOT-only return restored verified Ubuntu. Mac-independent Android-to-Ubuntu switching and repeated persistence/failure cycles remain. See [WAYDROID.md](WAYDROID.md), [DUALBOOT.md](DUALBOOT.md), and the [first-boot report](reports/native-android-firstboot-20260916.md). |
