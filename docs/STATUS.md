...
| Release packaging | In progress | The fourteen-package set is dependency-consistent under `t630-release-base` 0.1.15. First-boot 0.1.2 selects the real client WLAN deterministically and MAC-binds its saved profile; desktop and camera dependencies were refreshed with it. The installed tablet passes `dpkg --audit` and `apt-get check`. Boot v12 keeps the personalized root selected after orderly actions, preserves one-shot crash fallback, rejects charger/LPM WLAN startup, and passed exact write/readback, protected-neighbor verification, managed GNOME, Wi-Fi, and full Power Off acceptance. The ARM64 workflow physically produced a 3.5 GB identity-clean root with Settings, GNOME Software, native Firefox, LibreOffice, Xournal++, MyPaint, and all exact device packages, then sealed its 1,230,162,230-byte private archive, recovery runtime, and accepted BOOT input. A tablet-local stager copied the complete 1.2 GB bundle into bounded recovery RAM and reverified every hash. With the working system stopped and userdata genuinely unmounted, the read-only installer gate verified the userdata geometry, installed BOOT, protected recovery/vendor_boot/DTBO/VBMETA partitions, and isolated ARM64 tools without formatting. The exact DZE3/XAR factory ZIP also has a deep streaming verifier. Destructive clean-install/first-boot and return-to-stock acceptance remain behind their explicit gates. |
