...
| S Pen pressure | Confirmed in manual source trial; not default | Native GNOME GTK received 310 physical pen samples spanning normalized pressure 0–0.800534, plus 147 finger events, with the opt-in real-event/source-axis Mutter handoff trial. The owner confirms MyPaint pressure response, but excessive force and multi-second drawing lag remain. A failed 2× app sensitivity trial was reverted to identity. Failed proximity preloads remain removed. Physical hover-out lifecycle, responsive drawing, restart/rotation/lock regressions and safe packaging remain; normal startup has no pressure trial. See the [pressure-path report](reports/pen-pressure-path-20260915.md) |
| Pen apps | Evaluation | Native Xournal++ notes and MyPaint drawing launch in clean-root GNOME; continuous handwriting is visible. MyPaint has reproduced rendering and popup-grab faults; app-local single-worker/panel workarounds are installed. Its stroke queue had 7.405 s median/11.404 s p95 age under default-idle scheduling; an otherwise unchanged high-idle run measured 41/81 ms. The exact-version adapter now applies that app-local priority, with the original helper retained for rollback. Pressure feels much better. The remaining slight lag was tested separately from stock `classic/short_grass` smoothing by selecting pressure-aware, zero-tracking `deevad/ballpen` with an exact settings backup; physical acceptance is pending. Extended sessions, subjective pressure comfort, palm rejection, rotation, and save/reopen remain; Krita's native-Wayland rejection is documented |
