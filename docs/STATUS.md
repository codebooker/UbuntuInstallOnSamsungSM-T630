...
| Release packaging | In progress | Nine exact-version component packages and a release metapackage build reproducibly; the ten-file closure installed cleanly into an identity-safe Ubuntu 24.04.5 ARM64 rehearsal root and passed package, native-linkage, and mount-leak checks; factory-image preparation, remaining login/camera components, destructive boot, and recovery rehearsal remain |
