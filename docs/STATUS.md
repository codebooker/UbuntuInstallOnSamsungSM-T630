...
| Android applications | Container rejected; native dual boot in physical acceptance | Waydroid remains stopped because its llvmpipe UI is unusably slow. The physical storage split now provides 64 GiB Ubuntu `linuxroot` plus 44.2 GiB Android `userdata`; stock recovery successfully created and dry-run-checked Android F2FS and recreated `metadata` without changing Ubuntu or protected boot neighbors. Exact stock Android BOOT passed the no-write gate and a BOOT-only write/readback; first-boot display/setup and audited return switching remain. See [WAYDROID.md](WAYDROID.md), [DUALBOOT.md](DUALBOOT.md), and the [storage initialization report](reports/native-android-storage-init-20260916.md). |
