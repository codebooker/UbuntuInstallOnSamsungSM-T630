...
Last updated: 2026-09-17. Unless stated otherwise, results are from one physical
...
| Area | State | Notes |
| --- | --- | --- |
| Boot | Working | Stock downstream kernel with a replacement Ubuntu initramfs; Ubuntu root on the 64 GiB `linuxroot` partition |
...
| Keyboard | Working | Maliit/GNOME on-screen keyboard with Shift, Enter, Backspace, and Hide. Chrome sends text-input-v3 correctly, but nested Mutter does not surface its visibility request. Desktop 0.1.20 fixes Chrome 153 by pinning its explicit on-screen accessibility mode, regenerating the owner launcher after late package installs, and replacing any background Chrome process that predates the compatible flags. The replacement explicitly targets nested GNOME's `t630-gnome-0` socket, asks the Tablet Controls extension to activate Chrome, and closes the stale app-grid overlay; without those corrections it opened alive but invisibly beneath GNOME or behind its overview. A controlled focus test exposed the editable field through AT-SPI and opened GNOME's keyboard. Native application handling remains unchanged |
...
| Wi-Fi / remote access | Working | Normal boots use the stock CNSS timeout and connect the MAC-bound client radio as `wlan0`; first boot now treats successful association/DHCP as success even if best-effort profile hardening fails, and binds persistence with the exact sysfs client MAC. A physical restart automatically reconnected after this repair. The filesystem-ready shortcut remains diagnostic-only because charger/LPM boots exposed a regulatory null dereference on first scan. Boot v12 detects Samsung charger mode before consuming the release selector and cleanly reboots into normal mode. Owner-only SSH, real HTTPS, and the loopback screen feed start automatically. See the [cold-boot WLAN report](reports/cold-boot-wlan-charger-guard-20260916.md) |
...
| Speakers | Working | Stock calibration and guarded amplifier sequencing; GNOME volume control works. The clean first-boot path now registers audio before the sensor client sharing ADSP; a subsequent physical cold start exposed the real ALSA card and restored the 30% unmuted PipeWire sink |
...
| Release packaging | In progress | The exact-version set is now `t630-release-base` 0.1.29 with first-boot 0.1.3, desktop 0.1.20, and camera 0.1.7. A second clean install now starts from physically restored factory Android rather than an existing dual-boot layout: the guarded stock-origin splitter created only p34 `linuxroot` and p35 `userdata`, the installer formatted only p34, and the owner-created first boot reached GNOME. Bundle v8 passed its seal but physical app-drawer acceptance exposed that its replacement Chrome targeted the hidden parent compositor and left the overview above the replacement. Bundle v9 contains the corrected nested-GNOME display selection and explicit Shell activation; its complete clean Ubuntu Base build passed all seven sealed checksums and all six independently recomputed payload size/hash checks. Recovery initialization of the blank Android p35, next-session Shell activation acceptance, and the post-wipe button-only round trip remain the current physical gates. Signed standalone host-app packaging remains. See the [factory-baseline reinstall report](reports/factory-baseline-reinstall-20260917.md), [clean-install correction report](reports/clean-install-kernel-correction-20260917.md), [release refusal/recovery report](reports/release-refusal-recovery-gates-20260917.md), and [factory-return report](reports/full-factory-restore-20260917.md). |
...
| Android applications | Native Android working; corrected round trip accepted | The physical split provides 64 GiB Ubuntu `linuxroot` plus 44.2 GiB Android `userdata`. Stock Android 15 uses encrypted `/data`, Adreno 642L acceleration, Play Store/services, and Samsung Notes S Pen drawing. Ubuntu's **Restart into Android** confirmation and the no-network Android **Restart into Ubuntu** app completed a physical touchscreen round trip. A later endurance run found Android still carried the obsolete v1 Ubuntu image; it was replaced without a partition write by the module-compatible v2 image and a helper that pins physical `sda19`, flushes it, performs a delayed readback, and journals the handoff. The corrected round trip returned with GNOME, Wi-Fi, audio, Chrome keyboard support, clean packages, and zero checked faults. Bad-image-hash, no-external-power, and protected-neighbor mismatch drills all refused without a write or reboot. Rejected CPU-rendered Waydroid and its data were removed from Ubuntu. See [DUALBOOT.md](DUALBOOT.md), the [first-boot report](reports/native-android-firstboot-20260916.md), the [button-only round-trip report](reports/native-button-dualboot-20260917.md), and the [refusal report](reports/release-refusal-recovery-gates-20260917.md). |
...
- The accepted full factory restore erases the dual-boot installation and needs
  the exact device-specific pre-split GPT backup; the public project does not
  redistribute Samsung firmware or private GPT artifacts.
...
The complete private bundle, tablet-local RAM staging, and unmounted read-only
device gate are recorded in the
[full installer staging report](reports/full-installer-bundle-staging-20260916.md).
