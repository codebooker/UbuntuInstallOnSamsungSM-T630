...
| Release packaging | In progress | The fourteen-package native set is dependency-consistent under `t630-release-base` 0.1.16; desktop 0.1.7 adds on-demand Waydroid host preparation when its optional runtime is present. The installed tablet passes `dpkg --audit` and `apt-get check`. Boot v12 keeps the personalized root selected after orderly actions, preserves one-shot crash fallback, rejects charger/LPM WLAN startup, and passed exact write/readback, protected-neighbor verification, managed GNOME, Wi-Fi, and full Power Off acceptance. The ARM64 workflow refreshed the prior ownerless seal into a 3.5 GB identity-clean 0.1.16 root with Settings, GNOME Software, native Firefox, LibreOffice, Xournal++, MyPaint, and all exact device packages, then built and sealed its 1,230,162,607-byte private archive. The tablet-local stager copied that exact refreshed bundle into bounded recovery RAM and reverified every hash; the RAM copy was then unmounted and removed. The earlier 0.1.15 bundle also passed the full unmounted read-only device gate for userdata geometry, installed BOOT, protected recovery/vendor_boot/DTBO/VBMETA partitions, and isolated ARM64 tools. The refreshed bundle still requires the same unmounted gate before destructive acceptance. The exact DZE3/XAR factory ZIP also has a deep streaming verifier. Destructive clean-install/first-boot and return-to-stock acceptance remain behind their explicit gates. |
...
| Android applications | Working experimentally | The coherent v13 kernel/module payload passed native regressions and runs Waydroid 1.6.2 with an official GAPPS LineageOS 20 / Android 13 ARM64 system and MAINLINE vendor image. `sys.boot_completed=1`, Android DNS/HTTPS, nested-GNOME composition, clean shutdown/restart, Google Play services/check-in, the unauthenticated Play Store UI, and F-Droid 1.23.2 data persistence are verified. The integration package mounts the required cgroup-v1 controllers and binderfs, starts Android on demand, and launches LXC from the outer root so Android receives canonical trace descriptors. The Samsung downstream GPU ABI cannot accelerate the MAINLINE image, so 0.1.6 adds a reversible 1024×623 zero-animation software profile; it improves measured frame latency but remains llvmpipe CPU rendering. Google account sign-in, device registration, and a Play Store app install remain owner-only acceptance steps. The runtime is optional rather than part of the sealed offline base until its external packages and images are bundled. See [WAYDROID.md](WAYDROID.md). |
