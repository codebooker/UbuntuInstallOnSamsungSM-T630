...
| Android applications | Container rejected; native dual boot planned | Waydroid 1.6.2 reliably boots an official GAPPS LineageOS 20 / Android 13 ARM64 image, but Android SurfaceFlinger uses llvmpipe because neither upstream Freedreno nor the stock DZE3 EGL stack can use Waydroid's graphics path on this downstream kernel. The accepted 1024×623 zero-animation profile improves measurements but remains unusably slow in physical owner acceptance, so Waydroid is stopped and is not a daily-use solution. The retained stock `super`, factory images, 64 GiB Ubuntu / approximately 44 GiB Android split candidate, and BOOT-only switching design make native, GPU-accelerated Android the replacement direction. No partition change has been authorized or performed. See [WAYDROID.md](WAYDROID.md) and [DUALBOOT.md](DUALBOOT.md). |
