...
| Release packaging | In progress | The exact-version set is now `t630-release-base` 0.1.20 with desktop 0.1.11. The authorized clean install formatted only p34 `linuxroot`, extracted and checked the identity-clean root, and left Android p35 untouched. Its first boot exposed a release defect: the sealed v13 BOOT did not match the packaged v12 stock modules. BOOT v2 now uses the physically proven module-compatible v12 kernel with the unchanged dual-layout initramfs; full readback passed and the clean first-boot UI reached GNOME with touch, Wi-Fi hardware, and modules loaded. The private installer now requires and seals both one-button switch images instead of silently producing a root without them. Owner setup and a fresh post-fix Ubuntu→Android→Ubuntu round trip remain before the release gate can close. See the [clean-install correction report](reports/clean-install-kernel-correction-20260917.md). |
