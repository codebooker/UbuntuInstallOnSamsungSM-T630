...
| Pen apps | Evaluation | Native Xournal++ notes and MyPaint drawing launch in clean-root GNOME; continuous handwriting is visible. MyPaint has reproduced rendering and popup-grab faults; app-local single-worker/panel workarounds are installed, and the owner confirms physical touch/S Pen responsiveness recovered. Picker use, genuine pressure, palm rejection, rotation, and save/reopen remain; Krita's native-Wayland rejection is documented |
