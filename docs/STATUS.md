...
| Release packaging | In progress | The fourteen-package native source set is dependency-consistent under `t630-release-base` 0.1.17; desktop 0.1.8 adds automatic GNOME fullscreen for Waydroid surfaces. The installed tablet passes `dpkg --audit` and `apt-get check`. Boot v12 keeps the personalized root selected after orderly actions, preserves one-shot crash fallback, rejects charger/LPM WLAN startup, and passed exact write/readback, protected-neighbor verification, managed GNOME, Wi-Fi, and full Power Off acceptance. The last physically sealed ARM64 bundle remains the 3.5 GB identity-clean 0.1.16 root with Settings, GNOME Software, native Firefox, LibreOffice, Xournal++, MyPaint, and all exact device packages; its 1,230,162,607-byte private archive was copied into bounded recovery RAM, reverified, unmounted, and removed. The earlier 0.1.15 bundle also passed the full unmounted read-only device gate for userdata geometry, installed BOOT, protected recovery/vendor_boot/DTBO/VBMETA partitions, and isolated ARM64 tools. A new 0.1.17 bundle still requires sealing and the same unmounted gate before destructive acceptance. The exact DZE3/XAR factory ZIP also has a deep streaming verifier. Destructive clean-install/first-boot and return-to-stock acceptance remain behind their explicit gates. |
