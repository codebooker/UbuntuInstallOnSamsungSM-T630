...
| Release packaging | In progress | Ten exact-version component packages and a release metapackage build reproducibly; the eleven-file closure, including the Weston/Maliit boot runtime and lock guards, installed cleanly into an identity-safe Ubuntu 24.04.5 ARM64 rehearsal root and passed package, native-linkage, removal/restoration, and mount-leak checks; factory-image preparation, remaining login/camera components, destructive boot, and recovery rehearsal remain |
