...
| Keyboard | Working | Maliit/GNOME on-screen keyboard with Shift, Enter, Backspace, and Hide. Chrome sends text-input-v3 correctly, but nested Mutter does not surface its visibility request. Desktop 0.1.15 carries the owner-only AT-SPI editable-focus bridge and watches for Chrome being installed after login, closing the clean-install race that launched it without the required Wayland/accessibility flags. Native application handling remains unchanged |
