...
| Release packaging | In progress | Thirteen exact-version component packages and a release metapackage build reproducibly; the fourteen-file closure includes Weston/Maliit, lock guards, PolicyKit, GDM/elogind login, and the redistributable camera runtime, while clean-root provisioning supplies the normal Ubuntu application set; the identity-safe Ubuntu 24.04.5 ARM64 rehearsal root passes package, native-linkage, camera-boundary, app, and mount-leak checks; the packaged on-tablet preparer reconstructs private camera files directly from read-only stock `super`; boot-image assembly, destructive boot, and recovery rehearsal remain |
