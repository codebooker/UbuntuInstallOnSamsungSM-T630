...
| Pen apps | Evaluation | Native Xournal++ notes and MyPaint drawing launch in clean-root GNOME; continuous handwriting is visible. MyPaint has reproduced rendering and popup-grab faults; app-local single-worker/panel workarounds are installed, with physical recovery acceptance pending. Genuine pressure, palm rejection, rotation, and save/reopen remain; Krita's native-Wayland rejection is documented |
