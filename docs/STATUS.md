...
| Keyboard | Working | Maliit/GNOME on-screen keyboard with Shift, Enter, Backspace, and Hide. Chrome sends text-input-v3 correctly, but nested Mutter does not surface its visibility request. Desktop 0.1.16 fixes Chrome 153 by pinning its explicit on-screen accessibility mode, regenerating the owner launcher after late package installs, and retiring any background Chrome process that predates the compatible flags. A controlled focus test exposed the editable field through AT-SPI and opened GNOME's keyboard. Native application handling remains unchanged |
