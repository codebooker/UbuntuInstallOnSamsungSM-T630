...
| Release packaging | In progress | The thirteen-component set is dependency-consistent under `t630-release-base` 0.1.15. First-boot 0.1.2 selects the real client WLAN deterministically and MAC-binds its saved profile; desktop and camera dependencies were refreshed with it. The installed tablet passes `dpkg --audit` and `apt-get check`. Boot v12 keeps the personalized root selected after orderly actions, preserves one-shot crash fallback, rejects charger/LPM WLAN startup, and passed exact write/readback, protected-neighbor verification, managed GNOME, Wi-Fi, and full Power Off acceptance. A one-command ARM64 host workflow now emits and seals the identity-clean private rootfs, recovery-tool runtime, and accepted BOOT input without device I/O; the exact DZE3/XAR factory ZIP has a streaming recovery verifier. A constant-memory USB-serial stager verifies the whole bundle in a bounded tablet tmpfs without storage writes; its 96 MiB physical transfer/hash/cleanup test passed. The two-phase exact-userdata installer is implemented with read-only check mode and separate typed authorization. Full-size physical install/first-boot and return-to-stock acceptance remain. |
