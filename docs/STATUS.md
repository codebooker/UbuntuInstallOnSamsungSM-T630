...
| Pen apps | Evaluation | Native Xournal++ notes and MyPaint drawing launch in clean-root GNOME; continuous handwriting is visible. MyPaint has reproduced rendering and popup-grab faults; app-local single-worker/panel workarounds are installed. The owner confirms touch/S Pen recovery and the bounded long-stroke/brush/color control check. Pressure responds in the source trial but drawing lags by seconds; latency investigation is active. Extended sessions, comfortable pressure, palm rejection, rotation, and save/reopen remain; Krita's native-Wayland rejection is documented |
