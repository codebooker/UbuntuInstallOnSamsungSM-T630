...
| Keyboard | Working | Maliit/GNOME on-screen keyboard with Shift, Enter, Backspace, and Hide. Chrome sends text-input-v3 correctly, but nested Mutter does not surface its visibility request. Desktop 0.1.17 fixes Chrome 153 by pinning its explicit on-screen accessibility mode, regenerating the owner launcher after late package installs, and automatically replacing any background Chrome process that predates the compatible flags. The replacement avoids the apparent no-launch regression caused by terminating a process started from GNOME's cached old launcher. A controlled focus test exposed the editable field through AT-SPI and opened GNOME's keyboard. Native application handling remains unchanged |
...
| Release packaging | In progress | The exact-version set is now `t630-release-base` 0.1.26 with first-boot 0.1.3, desktop 0.1.17, and camera 0.1.7. A second clean install now starts from physically restored factory Android rather than an existing dual-boot layout: the guarded stock-origin splitter created only p34 `linuxroot` and p35 `userdata`, the installer formatted only p34, and the owner-created first boot reached GNOME. Bundle v7 verified the Chrome 153 accessibility mode, but physical app-drawer use exposed an apparent no-launch regression in its stale-process retirement. Bundle v8 contains the automatic compatible-process replacement; its complete clean Ubuntu Base build passed all seven sealed checksums and all six independent payload size/hash checks. Recovery initialization of the blank Android p35 and the post-wipe button-only round trip remain the current physical gates. Signed standalone host-app packaging remains. See the [factory-baseline reinstall report](reports/factory-baseline-reinstall-20260917.md), [clean-install correction report](reports/clean-install-kernel-correction-20260917.md), [release refusal/recovery report](reports/release-refusal-recovery-gates-20260917.md), and [factory-return report](reports/full-factory-restore-20260917.md). |
