...
| Keyboard | Working | Maliit/GNOME on-screen keyboard with Shift, Enter, Backspace, and Hide. Chrome sends text-input-v3 correctly, but nested Mutter does not surface its visibility request. Desktop 0.1.13 adds an owner-only AT-SPI editable-focus bridge to the existing GNOME keyboard action; the owner confirms Chrome fields now open the keyboard automatically, and an app-drawer launch after a clean restart retained the full flag set, watcher, extension, and visible keyboard. Native application handling remains unchanged |
...
| Release packaging | In progress | The exact-version set is now `t630-release-base` 0.1.22 with first-boot 0.1.3, desktop 0.1.13, and camera 0.1.7. The Chrome OSK focus bridge passed the real profile and clean-restart persistence gate. The authorized clean install formatted only p34 `linuxroot`, extracted and checked the identity-clean root, and left Android p35 untouched. Its first boot exposed a release defect: the sealed v13 BOOT did not match the packaged v12 stock modules. BOOT v2 now uses the physically proven module-compatible v12 kernel with the unchanged dual-layout initramfs; full readback passed and the clean first-boot UI reached GNOME with touch, Wi-Fi hardware, and modules loaded. The private installer now requires and seals both one-button switch images instead of silently producing a root without them. Sealed bundle v4 carries desktop 0.1.13 and release-base 0.1.22; its identity audit, full offline rehearsal, archive read-back, and seven-entry checksum verification passed. The stale Android return payload exposed by the endurance run was corrected, and the fresh Ubuntu→Android→Ubuntu cycle now passes on module-compatible BOOT v2. All three non-destructive forced-refusal classes pass. The destructive factory-return gate also passed: the exact pre-split GPT was restored, 40 stock payloads completed, and wiped stock Android re-enumerated. Signed standalone host-app packaging remains. See the [clean-install correction report](reports/clean-install-kernel-correction-20260917.md), [release refusal/recovery report](reports/release-refusal-recovery-gates-20260917.md), and [factory-return report](reports/full-factory-restore-20260917.md). |
