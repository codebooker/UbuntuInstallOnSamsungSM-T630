...
| Android applications | Working experimentally | The coherent v13 kernel/module payload passed native regressions and runs Waydroid 1.6.2 with an official VANILLA LineageOS 20 / Android 13 ARM64 image. `sys.boot_completed=1`, Android networking, nested-GNOME composition, clean shutdown, and F-Droid 1.23.2 install/persistence/launch are verified. The integration package mounts the required cgroup-v1 controllers and binderfs, starts Android on demand, and launches LXC from the outer root so Android receives canonical trace descriptors. It is optional rather than part of the sealed offline base until its external packages and images are bundled. See [WAYDROID.md](WAYDROID.md). |
