...
class DesktopRuntimePackageTests(unittest.TestCase):
    ...
    def test_build_is_reproducible_and_account_neutral(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("Version: 0.1.8", control)
            ...
            self.assertIn("'window-created'", extension)
            self.assertIn("window.make_fullscreen()", extension)
            self.assertIn("get_wm_class", extension)
            self.assertIn("Shell.WindowTracker", extension)
            self.assertIn("/proc/${pid}/cmdline", extension)
            self.assertIn("--desktop_file_hint=Waydroid.desktop", extension)
